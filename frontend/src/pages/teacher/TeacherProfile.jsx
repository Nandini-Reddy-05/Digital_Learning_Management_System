...
export const TeacherProfile = () => {
  ...
  const handleProfileSubmit = async (e) => {
    ...
    try {
      setError('');
      setSuccess('');
      const res = await api.put('/teacher/profile', profileForm);
      setProfile(res.data);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-sm space-y-6 flex flex-col items-center">
          ...
          <div className="text-center space-y-1">
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400">Instructor | {profile?.username}</p>
          </div>
            ...
            <form onSubmit={handleProfileSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" required value={profileForm.specialization} onChange={e => setProfileForm(prev=>({...prev, specialization: e.target.value}))} className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl py-2.5 px-3 outline-none text-sm font-medium"/>
                </div>
                <div>
                  <label className="block text-xs font-semibold mb-1 text-slate-500">Qualification</label>
                  <input type="text" required value={profileForm.qualification} onChange={e => setProfileForm(prev=>({...prev, qualification: e.target.value}))} className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl py-2.5 px-3 outline-none text-sm font-medium"/>
                </div>
              </div>
